...
serve(async (req) => {
    ...
    */
    ...
    try {
        ...
        // ⚡ PERMANENT CACHE — check pre-cached responses first (ANY profile, no TTL)
        // These are pre-generated by pre-cache-responses function and never expire.
        if (redisUrl && redisToken && activeProfileId) {
            const permKey = `perm:resp:${activeProfileId}:${normalizeCacheKey(query)}`;
            try {
                const permRes = await fetch(`${redisUrl}/get/${encodeURIComponent(permKey)}`,
                    { headers: { Authorization: `Bearer ${redisToken}` } }
                ).then(r => r.json());
                if (permRes.result) {
                    const cached = JSON.parse(permRes.result);
                    console.log(`⚡ [PERM-CACHE] HIT — "${query.slice(0, 50)}"`);
...
3. Return JSON: { "q1": "string", "q2": "string" }`
        ...
        // Mitesh-Khatri (the main/general persona) is the one exception to scope
        // discipline below — by design it's meant to answer from ANY program's
        // knowledge. Specific persona bots (Ho'oponopono, Chakra, NLP, etc.) stay
        // scoped to their own lane and redirect instead.
        const isMainPersona = !dynamicProfile?.name || /mitesh/i.test(dynamicProfile.name);
        const BASE_INSTRUCTIONS = [
            ...
            ...(isMainPersona ? [] : [
                `**PERSONA SCOPE DISCIPLINE (CRITICAL)**: You are specifically the **${dynamicProfile?.name}** coach — NOT the main Mitesh Khatri assistant. The KNOWLEDGE provided below is pulled from a shared library that covers ALL of Mitesh's programs (Chakra Healing, Wealth Mastery/PMDSPM, NLP, Daily Magic Practice/DMP, Relationship Mastery, Advance Law of Attraction, Ho'oponopono, Platinum, etc.) — even if a chunk surfaces with a good match score, that does NOT mean it's your subject. Before teaching ANY technique from the knowledge, check: does this actually belong to ${dynamicProfile?.name}'s own program? If the user's question clearly belongs to a DIFFERENT program than yours, do NOT teach that technique as your own — even if you have retrieved content for it. Instead, respond warmly and lovingly: acknowledge their question with care, gently let them know this is something the dedicated coach for that program goes deeper on, and softly guide them to switch to that coach inside the app — then bring the conversation back to what you, as their ${dynamicProfile?.name}, are here for. Never sound cold, robotic, or like a flat refusal — make it feel like a warm handoff between two coaches who both care about them, not a rejection.`
            ])
        ];
...
`,
        ...
        if (useFastSupportPath) {
            ...
            customInstructions = [
                ...
                ...(isMainPersona ? [] : [
                    `PERSONA SCOPE DISCIPLINE (CRITICAL): You are specifically the ${dynamicProfile?.name} coach — NOT the main Mitesh Khatri assistant. The KNOWLEDGE below is pulled from a shared library covering ALL of Mitesh's programs (Chakra Healing, Wealth Mastery/PMDSPM, NLP, DMP, Relationship Mastery, Advance LOA, Ho'oponopono, Platinum, etc.) — a high match score does NOT mean it's your subject. If the user's question clearly belongs to a DIFFERENT program than yours, do NOT teach that technique as your own even if you retrieved content for it. Instead respond warmly: acknowledge their question with care, let them know the dedicated coach for that program goes deeper on this, gently guide them to switch to that coach in the app, then bring the conversation back to what you, as their ${dynamicProfile?.name}, are here for. Never sound like a flat refusal — make it a warm handoff, not a rejection.`
                ]),
