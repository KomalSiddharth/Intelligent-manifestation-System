...
const ExperienceSettingsView = ({ profileId, initialData }: ExperienceSettingsViewProps) => {
    ...
    const [requireIdentityGate, setRequireIdentityGate] = useState(settings.requireIdentityGate ?? true);
    ...
    const loadProfile = async () => {
        try {
            ...
            // Resolved values (DB value OR fallback default)
            const resolvedInitialMessage   = settings.initialMessage   || "Hey Platinum Magician, this is Mitesh AI, how can i help you?";
            const resolvedNoAnswerMessage  = settings.noAnswerMessage  || "I'm sorry, I don't have the answer to that right now. In the meantime, feel free to explore our website at https://www.miteshkhatri.com or let me know if there's anything else I can assist with!";
            const resolvedDisclaimerText   = settings.disclaimerText   || "This is an AI assistant based on Mitesh's teachings and content. Responses are generated by AI and do not replace personalized coaching or professional advice.";

            setInitialMessage(resolvedInitialMessage);
            setNoAnswerMessage(resolvedNoAnswerMessage);
            ...
            setDisclaimerText(resolvedDisclaimerText);
            setRequireIdentityGate(settings.requireIdentityGate ?? true);

            // Auto-backfill: if initialMessage was never saved to DB, silently write the defaults
            // now so ChatPage can read them without the user having to manually hit "Save".
            if (!settings.initialMessage) {
                await updateMindProfile({
                    experience_settings: {
                        initialMessage:   resolvedInitialMessage,
                        noAnswerMessage:  resolvedNoAnswerMessage,
                        showCitations:    settings.showCitations    ?? true,
                        enableFileSending:settings.enableFileSending ?? true,
                        enableVoiceCalling: settings.enableVoiceCalling ?? true,
                        enableVideoCalling: settings.enableVideoCalling ?? false,
                        showDisclaimer:   settings.showDisclaimer   ?? true,
                        disclaimerText:   resolvedDisclaimerText,
                        requireIdentityGate: settings.requireIdentityGate ?? true,
                    }
                }, profileId);
                console.log('✅ [ExperienceSettings] Auto-backfilled defaults to DB for profile:', profileId);
            }
        } catch (error) {
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-4 pt-4 border-t">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <Label className="text-base">Identity Gate Protection</Label>
                                <span className="px-2 py-0.5 text-xs bg-orange-100 text-orange-600 rounded-full">Security</span>
                            </div>
                            <Switch checked={requireIdentityGate} onCheckedChange={setRequireIdentityGate} className="data-[state=checked]:bg-orange-500" />
                        </div>
                        <p className="text-sm text-muted-foreground">
                            Require users to verify their email (OTP) before they can start chatting with your clone.
                        </p>
                    </div>
